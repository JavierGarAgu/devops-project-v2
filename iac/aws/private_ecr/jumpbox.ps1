...
if (-not (Test-Path $adminKeyFile)) {
    terraform output -raw admin_private_key_pem | Set-Content -Path $adminKeyFile -Encoding ascii -Force
...
# Jumpbox private key setup (local, optional if you want to keep a local copy)
$jumpboxKeyFile = "$sshDir\jumpbox_id_rsa"
if (-not (Test-Path $jumpboxKeyFile)) {
    terraform output -raw jumpbox_private_key_pem | Set-Content -Path $jumpboxKeyFile -Encoding ascii -Force
    icacls $jumpboxKeyFile /inheritance:r | Out-Null
    icacls $jumpboxKeyFile /grant:r "$($env:USERNAME):(R)" | Out-Null
    icacls $jumpboxKeyFile /remove "Users" | Out-Null
} else {
    Write-Host "Jumpbox key $jumpboxKeyFile already exists"
}
